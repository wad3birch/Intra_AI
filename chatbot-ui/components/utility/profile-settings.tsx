...
export const ProfileSettings: FC<ProfileSettingsProps> = ({}) => {
  ...
  const handleSave = async () => {
    ...
    // Re-fetch all hosted models to ensure consistency
    // This avoids race conditions from multiple state updates
    const hostedModelRes = await fetchHostedModels(updatedProfile)
    if (hostedModelRes) {
      setEnvKeyMap(hostedModelRes.envKeyMap)
      setAvailableHostedModels(hostedModelRes.hostedModels)
    }

    // Handle OpenRouter models separately
    // Use the updated envKeyMap from the response if available
    const updatedEnvKeyMap = hostedModelRes?.envKeyMap || envKeyMap
    const hasOpenRouterKey =
      !!updatedProfile["openrouter_api_key"] || !!updatedEnvKeyMap["openrouter"]
    if (hasOpenRouterKey) {
