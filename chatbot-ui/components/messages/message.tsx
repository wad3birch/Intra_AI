...
import { MessageMarkdownDeepDive } from "./message-markdown-deep-dive"
import { MessageLearningActions } from "./message-learning-actions"
...
interface MessageProps {
  ...
  originalQuestion?: string // 添加原始问题参数
  chatId?: string // 添加chatId参数用于学习功能
}
...
export const Message: FC<MessageProps> = ({
  ...
  // 检查是否启用Deep Dive模式
  const isDeepDiveModeEnabled = chatSettings?.deepDiveEnabled || false
  ...
  return (
    ...
    >
        ...
        <div className="space-y-3">
          {message.role === "system" ? (
            ...
            <div className="flex items-center space-x-3">
              ...
              {/* Deep Dive 模式指示器 */}
              {message.role === "assistant" && isDeepDiveModeEnabled && (
          ...
          message.role === "assistant" ? (
            ...
            /* 根据Deep Dive模式设置选择不同的渲染组件 */
            message.role === "assistant" && isDeepDiveModeEnabled ? (
              <MessageMarkdownDeepDive 
      ...
      {/* Learning Actions */}
      {chatId && (
        <MessageLearningActions
          message={message}
          chatId={chatId}
          onCardGenerated={(card) => {
            // Handle card generation - could show a notification or update UI
            console.log('Knowledge card generated:', card)
          }}
        />
      )}
